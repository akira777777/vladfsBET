...
export function formatMoney(val: string | number | Decimal, decimals: number = 2): string {
  const d = new Decimal(val);
  return d.toFixed(decimals);
}

export function formatCrypto(val: string | number | Decimal): string {
  const d = new Decimal(val);
  return d.toFixed(8);
}

export function sumDecimals(...vals: (string | number | Decimal)[]): Decimal {
  return vals.reduce((acc: Decimal, v) => acc.plus(new Decimal(v)), new Decimal(0));
}
...
export function provablyFairStream(serverSeed: string, clientSeed: string, nonce: number, count: number): number[] {
  const floats: number[] = [];
  let round = 0;
...
// ----------------------------------------------------
// 1. PLINKO Provably Fair & Payout Matrices
// ----------------------------------------------------
...
export function calculatePlinkoDrop(
  serverSeed: string,
  clientSeed: string,
  nonce: number,
  rows: number,
  risk: PlinkoRisk
): { path: number[]; binIndex: number; multiplier: number } {
...
export function generateMinesLayout(
  ...
  nonce: number,
  mineCount: number
): { minePositions: number[]; grid: boolean[] } {
  const totalTiles = 25;
...
// ----------------------------------------------------
export function rollLimbo(serverSeed: string, clientSeed: string, nonce: number): number {
  const float = provablyFairFloat(serverSeed, clientSeed, nonce);
...
// ----------------------------------------------------
// 5. HILO (High-Low) Provably Fair & Multipliers
// ----------------------------------------------------
...
export function generateHiloDeck(serverSeed: string, clientSeed: string, nonce: number): HiloCard[] {
  const deck: HiloCard[] = [];
